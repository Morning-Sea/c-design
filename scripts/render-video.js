...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: actively reset animation time to zero — handles
    // HTML that does not strictly follow the starter tick template
    // (e.g. lastTick uses performance.now(), which folds font-loading time into the first frame's dt).
    ...
    if (seekCorrected) {
      // Wait two rAFs so the seek takes effect and the t=0 frame renders
